...
";
        ...
        private byte[] GetTileStream(string serviceName, string level, string row, string col, object otherParam)
        {
            Stopwatch sw = new Stopwatch();
            byte[] bytes = null;
            sw.Start();
            string key = Services[serviceName].Port + "_" + serviceName + "_" + level + "_" + row + "_" + col;
            Hashtable paramTable = otherParam as Hashtable;
            foreach (DictionaryEntry de in paramTable)
            {
                key += "_" + de.Value.ToString();
            }
            key += Services[serviceName].MemcachedValidKey;
        ...
        public Stream GetServiceOrigin(string serviceName)
        {
            string str = Services[serviceName].DataSource.TilingScheme.TileOrigin.X + "," + Services[serviceName].DataSource.TilingScheme.TileOrigin.Y;
            WebOperationContext.Current.OutgoingResponse.ContentType = "text/plain;charset=utf-8";
            return StreamFromPlainText(str, true);
        }
        public Stream GetVersions(string serviceName, string type, string startTime, string endTime)
        {
            string result = "";
            if (type == "hot")
            {
                result = BaiDuMapManager.inst.cp.getLatestVersionFormDB(startTime, endTime);
            }
            WebOperationContext.Current.OutgoingResponse.ContentType = "text/plain;charset=utf-8";
            return StreamFromPlainText(result, true);
        }
        public Stream GetPGISTile(string serviceName, string level, string row, string col)
        {
        ...
        public Stream GenerateBaiduTile(string serviceName, string level, string row, string col, string type, string time)
        {
            ...
            if (Services.ContainsKey(serviceName))
            {
                ...
                byte[] bytes = Task.Factory.StartNew<byte[]>(delegate ()
                {
                    Hashtable otherParam = new Hashtable();
                    otherParam.Add("TYPE", type);
                    otherParam.Add("TIME", time);
                    return GetTileStream(serviceName, level, row, col, otherParam);
                }).Result;
