...
namespace PBS.Util
{
    ...
    public static class Utility
    {
        ...
        public static Point LatLonToMeters(Point m)
        {
            /*Converts given lat/lon in WGS84 Datum to XY in Spherical Mercator EPSG:900913*/
            double mx = m.X * 20037508.3427892 / 180.0;
            double my = Math.Log(Math.Tan((90 + m.Y) * Math.PI / 360.0)) / (Math.PI / 180.0);
            my = my * 20037508.3427892 / 180.0;
            return new Point(mx, my);
        }
        public static Envelope GPSToWebMercator(Envelope gpsArea)
        {
            Util.Point pLeftTop = Utility.GeographicToWebMercator(new Util.Point(gpsArea.XMin, gpsArea.YMax));
            Util.Point pRightBottom = Utility.GeographicToWebMercator(new Util.Point(gpsArea.XMax, gpsArea.YMin));
            return new Envelope(pLeftTop.X, pRightBottom.Y, pRightBottom.X, pLeftTop.Y);
        }
